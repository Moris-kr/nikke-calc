...
it('binds external results to the request, model, complete job set and numeric totals',()=>{
 const packet=createModulePacket([job]);const row={currency:'modules',effect:0,value:264,keys:0,id:'a',mode:'complete-line',target:job.target,order:[0,1,2],lock:0,change:264,total:264,error95:0,samples:4000,unlocked:[]};
 ...
 expect(()=>parseModuleResult(JSON.stringify({...value,results:[{...row,effect:1}]}),packet)).toThrow();
 expect(()=>parseModuleResult(JSON.stringify({...value,results:[{...row,currency:'keys'}]}),packet)).toThrow();
});

...
export const MODULE_MODEL='2026-09-19-lock-keys-v2';
export interface ModuleJob{id:string;current:OverloadLine[];target:string[];locks:number;currency?:'modules'|'keys'}
...
잠금 비용 첫 줄 1개/추가 둘째 줄 2개, 변경 비용 1+잠금 수, 동일 수치 재추첨은 무료이며 다른 수치가 나올 때까지 반복합니다. 6배치×6순서×2전략 중 표본 평균으로 선택한 전략을 독립 표본 4000개로 평가합니다. 전역 최적 보장이 아니며 error95는 기대값 추정 오차이지 실제 소모량 범위가 아닙니다. currency=keys는 기존 모듈 잠금을 해제하고 매 변경마다 1줄 20개/2줄 총 50개의 락 키로 다시 잠급니다. 변경 모듈은 동일하게 1+잠금 수입니다. effect는 효과변경, value는 수치변경 모듈이며 keys는 별도 재화로 합산하지 않습니다.
<run-battle.py>
${battleRunner}
</run-battle.py>
<solver.mjs>
...
export function parseModuleResult(raw:string,packet:ModulePacket):Map<string,ModuleRoute>{
 ...
 for(const row of value.results){
  ...
  for(const key of ['lock','change','total','error95','effect','value','keys'])if(typeof row[key]!=='number'||!Number.isFinite(row[key])||row[key]<0||row[key]>1e7)throw new Error('모듈 수치가 올바르지 않습니다.');
  if(row.currency!==(job.currency??'modules')||Math.abs(row.change-row.effect-row.value)>1e-6||(row.currency==='modules'&&row.keys!==0)||(row.currency==='keys'&&row.lock!==0))throw new Error('잠금 재화 또는 비용 분류가 올바르지 않습니다.');
  ...
  rows.set(row.id,{currency:row.currency,effect:row.effect,value:row.value,keys:row.keys,mode:row.mode,target:[...row.target],order:[...row.order],lock:row.lock,change:row.change,total:row.total,error95:row.error95,samples:row.samples,unlocked:[...row.unlocked]});
 }
 return rows;
}

 ...
 * 자리가 여전히 의미 있는 것은 같은 단계 버스트 우선순위(앞자리 먼저)와 후열·양옆 아군 스킬뿐이다.
 */
...
describe('편성 자리와 무관한 결과', () => {
  it('같은 단계 버스트끼리의 앞뒤만 지키면 자리를 바꿔도 총딜·캐릭터별 딜이 같다', () => {
    ...
    const orders = [[M, P, S, C, L], [M, C, P, S, L], [M, L, P, C, S], [P, S, M, C, L]];

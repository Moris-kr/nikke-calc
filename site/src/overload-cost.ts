...
export interface ModuleRoute {mode:'complete-line'|'effects-first';target:string[];order:number[];lock:number;change:number;total:number;error95:number;samples:number;unlocked:number[]}
...
export function evaluateRoute(initial:OverloadLine[],target:string[],order:number[],initialLocks:number,rng:()=>number,mode:'complete-line'|'effects-first'='complete-line'):{lock:number;change:number}{
  const state=initial.map(row=>({...row}));let locks=initialLocks;let lock=0,change=0;
  ...
  for(const slot of order){
    ...
    if(mode==='complete-line'&&state[slot]!.level!==15){
      ...
      change+=cost*(1-LEVEL_PROB[state[slot]!.level-1]!)/LEVEL_PROB[14]!;
      for(let i=0;i<3;i++)if(!(locks&(1<<i))&&state[i]!.option)state[i]!.level=i===slot?15:level(rng,state[i]!.level);
    }
    if(!order.every(done)){lock+=count(locks)+1;locks|=1<<slot;}
  }
  if(mode==='effects-first'){
    ...
    while(state.some(row=>row.level!==15)){
      for(let i=0;i<3;i++)if(state[i]!.level===15&&!(locks&(1<<i))){lock+=count(locks)+1;locks|=1<<i;}
      ...
      change+=(1+count(locks))/outcomes.reduce((sum,outcome)=>sum+outcome.prob,0);
...
export function estimateModules(current:OverloadLine[],options:string[],initialLocks=0,samples=4000):ModuleRoute{
  ...
  const run=(target:string[],order:number[],n:number,seed:number,mode:ModuleRoute['mode'])=>{
    const rng=seeded(seed);let lock=0,change=0,sum2=0;
    for(let i=0;i<n;i++){const cost=evaluateRoute(current,target,order,initialLocks,rng,mode);lock+=cost.lock;change+=cost.change;sum2+=(cost.lock+cost.change)**2;}
    const total=(lock+change)/n;
    return {mode,target,order,lock:lock/n,change:change/n,total,error95:n>1?1.96*Math.sqrt(Math.max(0,(sum2-n*total*total)/(n-1))/n):0,samples:n,
      unlocked:[0,1,2].filter(i=>(initialLocks&(1<<i))&&(current[i]!.option!==target[i]||current[i]!.level!==15))};
  };

...
class BrowserBridgeTest(unittest.TestCase):
    ...
    def test_bundled_temporary_characters_simulate_with_fiction_warning(self):
        ...
        result = json.loads(run_request(json.dumps(payload, ensure_ascii=False)))
        self.assertTrue(result["timeline"]["fullBurst"])
        self.assertIn("[임시 · 창작]", result["previewNote"])
        for name in names:
            self.assertIn(name, result["previewNote"])
            self.assertTrue(result["timeline"]["bursts"][name])
            self.assertEqual(result["timeline"]["bursts"][name][0]["stage"], "3")
            self.assertIn("[창작]", result["timeline"]["bursts"][name][0]["skill"])

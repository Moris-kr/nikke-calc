...
| `N중첩` | 해당 clause 직전 효과 항목의 `max_stack`으로 기록. **직전 항목이 `dot_damage`면 `"scaling": "stack_count"`도 함께 적는다** — `[N 중첩]` DoT는 인스턴스가 병존하므로(`GAMEPLAY.md` §버프 스택) 틱 대미지가 중첩만큼 곱해져야 하는데, 엔진은 그 표시가 있을 때만 곱한다(`timeline.ts`). 빠뜨리면 중첩은 쌓이는데 대미지는 1중첩에 머물며 로그에도 흔적이 없다 (레이븐 `쇼크웨이브`가 그랬다 — 총딜 −208%). `site/src/engine/tests/stacking_dot.test.ts`가 강제한다 |
...
| `자신이 [상태명] 상태라면` | `"self_state:상태명"` — **상태명은 그 상태를 «거는 효과의 이름»이다.** 아래 §self_state가 가리키는 것 |
...
**원문 대괄호 안의 상태 이름을 그대로 옮기면 안 된다.** `buff_manager._has_self_state`가
이름을 푸는 길은 둘뿐이다:

1. `_by_name(X)` — 이름이 X인 **활성 효과**에 이 캐릭터가 들어 있나 (아군이 걸어 준 것도 풀린다)
2. `weapon_change_name(caster) == X` — 무기 변경 모드 이름

그래서 X는 **데이터 어딘가에 `name`으로 존재해야** 한다. 원문이 `[평정심 : 크리티컬 확률
n% ▲]`처럼 대괄호 안에 상태 이름을 적어 두더라도, 그 이름으로 등록되는 효과는 없다 —
효과 이름은 스킬 이름(`바이팅 리더십`)이다. 조건은 **그 스킬 이름**을 가리켜야 한다.

| 원문 | ✗ 안 되는 것 | ✓ 맞는 것 |
|---|---|---|
| 목단 `자신이 무기 변경 상태라면` | `self_state:무기 변경` | `self_state:정정당당 승부다!` (모드 이름) |
| 율하 `자신이 평정심 상태라면` | `self_state:평정심` | `self_state:바이팅 리더십` (거는 스킬) |
| 메이든 `자신이 복수 상태라면` | `self_state:복수` | `self_state:언령 : 기교의 언` |

틀리면 **조용히 죽는다** — 예외도 경고도 없이 조건이 영원히 거짓이라 딜만 낮게 나온다.
목단 「다 덤벼!」의 5타 추가 대미지가 그렇게 죽어 있었고(개인 딜 −38%), 실측 제보로 겨우
드러났다. `site/src/engine/tests/state_conditions.test.ts`가 이제 데이터 전체를 훑어 막는다.

**상태를 만드는 것은 `buff`뿐이다.** `instant`는 `_dispatch_instant` 뒤 곧바로 반환되어
`_active`에 안 들어가므로, 아무리 `duration`을 적어도 `self_state:`로 조회되지 않고
`event:state_end:`도 나오지 않는다. 원문 `[미래 예지]`처럼 **수치 없이 이름만 있는 상태**는
`stat` 없는 중립 마커 버프로 둔다(민트 `무대 파트 : 댄스`, 밀크 `부끄러움` 선례):

```jsonc
{ "type": "buff", "name": "미래 예지", "trigger": {"timing": ["burst_cast"], "condition": []},
  "target": "self", "polarity": "neutral", "max_stack": 1, "duration": 10 }
```

같은 이유로 `remove_named_buff`의 `target_effect`도 **상태를 지고 있는 효과**를 가리켜야
한다. 원문에 `[스타 게이저 제거]`라고 적혀 있어도, 우리 데이터에서 그 상태를 지고 있는 것이
`스타게이저 2`면 그쪽을 적는다.

##### `target_state:`는 적에게 붙은 것만 본다

`_has_target_state`는 `"__enemy__"`가 대상인 활성 효과만 훑는다. 아군·자신에게 붙는
효과를 가리키면 **언제나 거짓**이다.

| 원문 | ✗ | ✓ |
|---|---|---|
| 백학 `회복 독점 서약 상태인 아군 전체에게` | `target_state:회복 독점 서약` (아군 버프다) | 대상 자체를 `allies_with_buff:회복 독점 서약`으로 |
| 베스티 `대상이 내부 폭발탄 상태라면` | `target_state:내부 폭발탄` (은화 **자신**의 무기 변경이다) | `target_state:내부 폭발탄 2` (적에게 붙는 쪽) |
| 솔린 `대상이 티켓 보유 상태라면` | `target_state:티켓` (게이지다) | `gauge_above:티켓:1` |

같은 이름이 자기 쪽과 적 쪽에 하나씩 있는 경우가 잦다 — 무기 변경 모드(자신)와 그 모드가
적에게 거는 디버프가 원문에서 같은 이름을 쓰기 때문이다. **적에게 붙는 쪽의 우리 이름**을
확인하고 적는다.
...
`self_state:[모드명]` / `not_self_state:[모드명]`이 성립한다 — 일반 named buff와 같다. 원문이
**모드 이름 대신 일반명**(`자신이 무기 변경 상태라면` — 목단 「다 덤벼!」)으로 쓰더라도
`self_state:`에는 **그 캐릭터의 실제 모드 이름**을 적는다. 일반명을 그대로 옮기면 어떤
이름과도 안 맞아 조건이 영원히 거짓이 된다.

...
STATUS_EXEMPT: dict[str, str] = {
    ...
    "decoy": "데이터에 분신 원문을 보존하지만 분신 피격 모델은 없어 ❌가 맞다",
    "enemy_movement_disable": "데이터에 이동 불가 원문을 보존하지만 적 이동 모델은 없어 ❌가 맞다",
    "explosion_range": "데이터에 폭발 범위 원문을 보존하지만 단일 적 위치·범위 모델은 없어 ❌가 맞다",
    "infinite_ammo": "기존 데이터 키는 미구현으로 유지한다. 신규 구현 키 `max_ammo_infinite`의 부분 문자열과 휴리스틱이 겹친다",
    "intercept_dmg_pct": "저지 부위 모델이 없어 의도적으로 미구현이다. 배치 테스트는 데이터 보존 여부만 확인한다",
    "atk_copy": "공격력 복제는 미구현이며 배치 테스트의 데이터 보존 assertion 문자열만 존재한다",
}

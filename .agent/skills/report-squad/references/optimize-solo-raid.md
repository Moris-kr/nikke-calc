...
- 새 시뮬은 **첫 후보 원본의 시드셋·반복 수·전역 육성/랩쳐 조건**을 그대로 물려받는다.
  캐시 결과와 합산해도 시드가 짝지어진다. 결과는 `.report-work/<슬러그>/pinned.data.json`에
  캐시되며, 계산기 코드·데이터가 바뀌면 자동으로 다시 돈다.
- `sources` 없이 `pinned_squads`만 두면 후보 없이 전부 새로 시뮬한다. 이때 시드·반복은
  스펙의 `runs`, 육성·설정은 스펙의 `defaults`·`config`·`enemy`를 쓴다.

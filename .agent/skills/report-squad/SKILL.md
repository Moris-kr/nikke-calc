...
나머지는 기존 결과를 재사용한다. 계산기 코드·공용 스펙·파싱 데이터 또는 난수 모드·시드·
반복 조건이 바뀌면 자동으로 전체 재계산한다. 유저가 처음부터 재계산을 요청했을 때만 `--full`을 쓴다.
...
1. 생성된 HTML 경로를 알려준다.
2. 케이스별 총딜을 억 단위로 짧게 적는다. 기본은 기대값 모드(난수 없음)라 ±가 없다 —
   `--sampled`로 돌렸을 때만 ± 표준편차를 함께 적는다.
3. 기본 스펙 이탈 내용을 답변에도 옮긴다. 없으면 `1층 그대로`라고 적는다.
4. 예상 밖 순위·기여도 독식처럼 눈에 띄는 점만 한두 줄 덧붙인다 (CV는 `--sampled`일 때만).

...
같은 자료를 주므로, 이름만 세 벌 받아 두면 화면이 그 사람의 말로 부를 수 있다.
...
def collect_roles(ids: list[int]) -> tuple[dict, dict]:
    ...
    def one(rid: int) -> None:
        try:
            pack = {
                locale: fetch_json(ROLEDATA_PATH.format(rid=rid, locale=locale))
...
def collect_equip() -> tuple[dict, dict]:
    """큐브·애장품 이름."""
    ...
    for entry in cube_map:
        ...
        try:
            pack = {
                locale: fetch_json(CUBE_PATH.format(locale=locale, cid=cid))
                for locale in ("ko", *LOCALES)
            }
        except (urllib.error.HTTPError, urllib.error.URLError):
            continue
    ...
    def one(fid) -> None:
        try:
            pack = {
                locale: fetch_json(FAVORITE_PATH.format(locale=locale, fid=fid))
                for locale in ("ko", *LOCALES)
            }
        except (urllib.error.HTTPError, urllib.error.URLError):
            return
...
def run(check: bool) -> int:
    ids = role_ids()
    characters, skills = collect_roles(ids)
    cubes, favorites = collect_equip()

    fresh = {
        "_comment": "영어·일본어 이름. `scraper/cdn_locale.py`가 CDN에서 받아 적는다.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="영어·일본어 이름 수집")
